...
type Config = {
  ...
  scheduleDate: string;
  // 12-hour time: { hour: "01".."12", minute: "00".."59", meridiem: "AM" | "PM" }
  scheduleHour: string;
  scheduleMinute: string;
  scheduleMeridiem: "AM" | "PM";
  liveUntil: string; // always, 1week, 2weeks, 3weeks, 1month, custom
  endDate: string;
  endHour: string;
  endMinute: string;
  endMeridiem: "AM" | "PM";
...
// Convert 12-hour (h, mm, AM/PM) -> 24-hour HH:MM
...
export default function PreviewPublish() {
  ...
  if (loading || !test)
    return <div className="p-8 text-slate-500 text-center">Loading...</div>;
  ...
  const publish = async () => {
    ...
    if (config.publishMode === "scheduled") {
      ...
      // If the scheduled time is in the future, status becomes "scheduled"
    ...
    // - Otherwise (Publish Now or past time) -> "live"
    ...
    const message =
      previouslyPublished
        ? isFutureScheduled
          ? "Test rescheduled successfully!"
          : "Test re-published successfully!"
        : isFutureScheduled
    ...
    // Update local test object so the UI reflects new status
        ...
        <aside className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          <div className="p-4 border-b border-slate-100">
            ...
            <div className="text-xs text-slate-500 mt-1">
              Total Questions:{" "}
              <span className="text-slate-800 font-semibold">{totalQ}</span>
            </div>
          </div>
          <div className="p-3 space-y-1.5 max-h-[600px] overflow-y-auto">
            ...
            {questions.map((q: any, i: number) => (
              <QItem key={q.id || i} num={i + 1} />
            ))}
          </div>
        </aside>

        {/* RIGHT MAIN - Confirmation */}
        <section className="bg-white rounded-xl border border-slate-200 p-6 space-y-6">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {(test.status === "live" || test.status === "scheduled") && (
                <span className="text-xs text-amber-700 bg-amber-50 border border-amber-200 px-2.5 py-1 rounded-md">
                  This test is already {test.status === "live" ? "published" : "scheduled"}. Confirming again will update its schedule and status.
                </span>
              )}
            </div>
            <button
              onClick={() => navigate(`/test-creation/${id}`)}
              title="Edit Test"
              className="w-8 h-8 flex items-center justify-center text-indigo-600 hover:bg-indigo-50 rounded-md"
            >
          ...
          <div className="border border-slate-200 rounded-xl p-5">
            <div className="flex flex-wrap items-center gap-2 mb-3">
              <span className="inline-flex items-center gap-1.5 bg-indigo-950 text-white text-[11px] font-semibold px-2.5 py-1 rounded-md">
                {test.type === "mock" ? "Mock Test" : test.type === "pyq" ? "PYQ" : "Chapter Wise"}
              </span>
              <span className="inline-flex items-center gap-1.5 bg-indigo-900/10 text-indigo-700 text-[11px] font-semibold px-2.5 py-1 rounded-md border border-indigo-200">
                <svg width="8" height="8" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
                </svg>
                Chapter 1
              </span>
              <span className="inline-flex items-center bg-emerald-500 text-white text-[11px] font-semibold px-2.5 py-1 rounded-md">
                <svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor" className="mr-1">
                  <circle cx="5" cy="12" r="1.5" />
                  <circle cx="12" cy="12" r="1.5" />
                  <circle cx="19" cy="12" r="1.5" />
                </svg>
                {(test.difficulty || "easy").charAt(0).toUpperCase() +
                  (test.difficulty || "easy").slice(1)}
              </span>
            </div>
            ...
            <div className="space-y-2 text-sm">
              ...
              <MetaRow
                label="Topic"
                pills={(test.topics_names || []).length ? test.topics_names : ["—"]}
                variant="amber"
              />
              <MetaRow
                label="Sub Topic"
          ...
          <div className="inline-flex bg-slate-100 rounded-lg p-1">
            <button
              onClick={() => setConfig({ ...config, publishMode: "now" })}
              className={`px-4 py-2 text-xs font-medium rounded-md transition ${
                config.publishMode === "now"
            ...
            <button
              onClick={() => setConfig({ ...config, publishMode: "scheduled" })}
              className={`px-4 py-2 text-xs font-medium rounded-md transition ${
          ...
          {config.publishMode === "scheduled" && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <label className="block text-sm">
                <span className="block text-slate-700 mb-1.5 text-xs font-medium">
                  Select Date
                </span>
                <input
                  type="date"
                  ...
                  onChange={(e) =>
                    setConfig({ ...config, scheduleDate: e.target.value })
                  }
              ...
              <label className="block text-sm">
                ...
                <TimeInput12
                  ...
                  meridiem={config.scheduleMeridiem}
                  onChange={(hour, minute, meridiem) =>
                    setConfig({ ...config, scheduleHour: hour, scheduleMinute: minute, scheduleMeridiem: meridiem })
                  }
                />
              </label>
            </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3">
              <RadioRow
                ...
                checked={config.liveUntil === "always"}
                onChange={() => setConfig({ ...config, liveUntil: "always" })}
              />
              <RadioRow
                label="3 Weeks"
                checked={config.liveUntil === "3weeks"}
                onChange={() => setConfig({ ...config, liveUntil: "3weeks" })}
              />
              <RadioRow
                label="1 Week"
                checked={config.liveUntil === "1week"}
                onChange={() => setConfig({ ...config, liveUntil: "1week" })}
              />
              <RadioRow
                label="1 Month"
                checked={config.liveUntil === "1month"}
                onChange={() => setConfig({ ...config, liveUntil: "1month" })}
              />
              <RadioRow
                label="2 Weeks"
                checked={config.liveUntil === "2weeks"}
                onChange={() => setConfig({ ...config, liveUntil: "2weeks" })}
              />
              <RadioRow
                label="Custom Duration"
                checked={config.liveUntil === "custom"}
                onChange={() => setConfig({ ...config, liveUntil: "custom" })}
              />
            </div>
          </div>
          ...
          {config.liveUntil === "custom" && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1">
              <label className="block text-sm">
                <input
                  type="date"
                  min={config.scheduleDate || todayISO()}
                  value={config.endDate}
                  onChange={(e) => setConfig({ ...config, endDate: e.target.value })}
                  placeholder="Select End Date"
                  className="w-full px-3 py-2.5 border border-slate-200 rounded-lg text-sm text-slate-500 bg-white"
                />
              </label>
              <label className="block text-sm">
                <TimeInput12
                  ...
                  meridiem={config.endMeridiem}
                  onChange={(hour, minute, meridiem) =>
                    setConfig({ ...config, endHour: hour, endMinute: minute, endMeridiem: meridiem })
                  }
                />
              </label>
            </div>
          ...
          <div className="flex justify-end gap-3 pt-2 border-t border-slate-100">
            <button
              onClick={() => navigate("/")}
              className="px-5 py-2.5 text-sm font-medium text-indigo-600 bg-white border border-slate-200 rounded-lg hover:bg-slate-50"
            >
              Cancel
            </button>
            <button
              onClick={publish}
              className="px-8 py-2.5 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg shadow-sm"
            >
              Confirm
            </button>
          </div>
...
function QItem({ num }: { num: number }) {
  return (
    <div className="flex items-center gap-2 px-2 py-2 rounded-lg hover:bg-slate-50 cursor-pointer text-xs">
      <span className="w-5 h-5 shrink-0 rounded-full bg-emerald-100 text-emerald-700 font-bold text-[10px] flex items-center justify-center border border-emerald-200">
        ✓
      </span>
    ...
    <label className="flex items-center gap-3 cursor-pointer py-1">
      <span className="relative inline-flex">
        <input
          ...
          checked={checked}
          onChange={onChange}
          className="peer appearance-none w-4 h-4 rounded-full border-2 border-indigo-400 checked:border-indigo-600"
        />
...
function StatusPill({ status }: { status?: string }) {
  ...
  if (status === "scheduled") {
    return (
      <span className="inline-flex items-center gap-1.5 bg-sky-50 border border-sky-200 text-sky-700 text-[11px] font-medium px-2.5 py-1 rounded-full">
  ...
  return (
    <span className="inline-flex items-center gap-1.5 bg-amber-50 border border-amber-200 text-amber-700 text-[11px] font-medium px-2.5 py-1 rounded-full">
...
// 12-hour time input — lets the user pick hour, minute, and AM/PM manually
function TimeInput12({
  ...
  return (
    <div className="flex items-stretch gap-1">
      <select
        value={hour}
        onChange={(e) => onChange(e.target.value, minute, meridiem)}
        className="flex-1 px-3 py-2.5 border border-slate-200 rounded-lg text-sm bg-white text-slate-700"
      >
        {hours.map((h) => (
      ...
      <select
        value={minute}
        onChange={(e) => onChange(hour, e.target.value, meridiem)}
        className="flex-1 px-3 py-2.5 border border-slate-200 rounded-lg text-sm bg-white text-slate-700"
      >
        {minutes.map((m) => (
      ...
      <select
        value={meridiem}
        onChange={(e) => onChange(hour, minute, e.target.value as "AM" | "PM")}
        className="px-3 py-2.5 border border-slate-200 rounded-lg text-sm bg-white text-slate-700 font-medium"
      >
