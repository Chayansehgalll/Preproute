...
export default function CreateTest() {
  ...
  useEffect(() => {
    if (test.subject) {
      api.getTopics(test.subject).then((r: any) => {
        setTopics(r.data);
        setTest((prev) => ({ ...prev, topics: [], sub_topics: [] }));
        setSubTopics([]);
      });
    } else {
      setTopics([]);
  ...
  const submit = async (redirectToQuestions: boolean) => {
    ...
    try {
      if (isEdit) {
        ...
        setSavedMsg("Test saved as draft!");
      }
    } catch (err: any) {
  ...
  const increment = (k: "correct_marks" | "wrong_marks" | "unattempt_marks" | "total_marks" | "total_questions" | "total_time", by: number) => {
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          <button onClick={() => navigate("/")} className="text-slate-500 hover:text-slate-700 text-sm flex items-center gap-1">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M19 12H5M12 19l-7-7 7-7" /></svg>
            Back
          </button>
        </div>
        <h1 className="text-2xl font-bold text-slate-800">{isEdit ? "Edit Test" : "Create New Test"}</h1>
        <p className="text-slate-500 text-sm mt-1">Configure your test settings. You can add questions after this step.</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-8">
        ...
        <div className="mt-8">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-semibold text-slate-800">Marking Scheme</h3>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            <NumberInput label="Wrong Answer" value={test.wrong_marks} onChange={(v: number) => update("wrong_marks", v)} onInc={() => increment("wrong_marks", 1)} onDec={() => increment("wrong_marks", -1)} />
            <NumberInput label="Unattempted" value={test.unattempt_marks} onChange={(v: number) => update("unattempt_marks", v)} onInc={() => increment("unattempt_marks", 1)} onDec={() => increment("unattempt_marks", -1)} />
            <NumberInput label="Correct Answer" value={test.correct_marks} onChange={(v: number) => update("correct_marks", v)} onInc={() => increment("correct_marks", 1)} onDec={() => increment("correct_marks", -1)} positive />
            <div>
        ...
        {errors._ && <div className="mt-4 text-sm text-red-600 bg-red-50 border border-red-200 rounded-md px-3 py-2">{errors._}</div>}
        {savedMsg && <div className="mt-4 text-sm text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-md px-3 py-2">{savedMsg}</div>}
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={`transition ${open ? "rotate-180" : ""}`}><path d="M6 9l6 6 6-6" /></svg>
      </button>
      {open && (
        <>
          ...
          <div className="absolute z-20 mt-1 w-full bg-white border border-slate-200 rounded-lg shadow-lg max-h-52 overflow-y-auto">
            {options.length === 0 && <div className="p-3 text-sm text-slate-400">No options available</div>}
            {options.map((opt: any) => {
              const checked = selected.includes(opt.value);
              return (
                <label key={opt.value} className="flex items-center gap-2 px-3 py-2 hover:bg-slate-50 cursor-pointer text-sm">
                  <input type="checkbox" checked={checked} onChange={() => onToggle(opt.value)} className="text-indigo-600" />
    ...
    <div>
      ...
      <div className="flex items-stretch border border-slate-200 rounded-lg overflow-hidden bg-white">
        ...
        <div className="flex flex-col border-l border-slate-200">
          <button type="button" onClick={onInc} className="px-2 text-slate-500 hover:bg-slate-50 border-b border-slate-200 flex-1">
            <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M6 15l6-6 6 6" /></svg>
          </button>
          <button type="button" onClick={onDec} className="px-2 text-slate-500 hover:bg-slate-50 flex-1">
